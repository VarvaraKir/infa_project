#include "Grid.hpp"
Grid::Grid(int sum_column, int sum_line, double D, double dx, double dt)
{
    ...
    this->rho = a[0][0].get_rho();

    // Наливаем раствор в центральную область
    unsigned center = sum_line / 2;
    unsigned k = 40;
    int m = 0;
    for (unsigned i = center - k; i < center + 1; i++)
    {
        for (unsigned j = center - m; j < center + m + 1; j++)
        {
            int position = (center - i)*(center -i) + (center - j)*(center -j);
            int factor = 1 - position/(k*k);
            if (factor<0) std::cout<<"oh no"<<std::endl;
            a[i][j].setC(rho*factor);
        }
        m++;
    }
    // std::cout<<m<<std::endl;
    for (unsigned i = center + 1 ; i < center + k + 1; i++)
    {
        for (unsigned j = center - m; j < center + m + 1; j++)
        {
            a[i][j].setC(rho);
